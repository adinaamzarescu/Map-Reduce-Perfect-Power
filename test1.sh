...
function show_score {
	...
	echo "Score total:       $((correctness + scalability))/120"
}
...
function run {
	...
	if [ $ret == 124 ]
	then
		echo "W: The program took more than 120s"
	elif [ $ret != 0 ]
	then
...
function run_and_get_time {
	...
	if [ $ret == 124 ]
	then
		echo "E: The program took more than 120s"
		cat time.txt | sed '$d' | sed '$d' | sed '$d'
		show_score
		rm -rf test*_sec
		exit
...
function run_par_and_measure {
	...
	if [ $ret == 124 ]
	then
		echo "W: The program took at least 2 secons more than expected"
	elif [ $ret != 0 ]
	then
		echo "W: Couldn't execute"
		cat time.txt | sed '$d' | sed '$d' | sed '$d'
...
do
	...
	do
		...
		if [ $P == 2 ]
		then
			echo "It took ${par_times[$i * 2]} seconds"
		else
			echo "It took ${par_times[$i * 2 + 1]} seconds"
		fi
	...
	then
		...
		then
			speedup12=0
			speedup14=0
			echo "Wrong answer, speedup is 0"
		fi
		...
		then
			...
			then
				...
				echo "W: Speedup from 1 to 2 Mappers too low"
			else
				echo "W: Speedup from 1 to 2 Mappers too low"
			fi
		fi
		...
		then
			scalability=$((scalability+7))
			echo "W: Speedup from 1 to 4 Mappers too low"
		else
			echo "W: Speedup from 1 to 4 Mappers too low"
		fi
	fi
